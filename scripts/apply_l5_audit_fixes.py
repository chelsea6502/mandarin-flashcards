"""Apply L5 fresh-audit fixes from all chunk fix files to words.xlsx."""
import json
import glob
import os
import openpyxl
...
# Load all fix files in order
fix_files = sorted(glob.glob(os.path.join(FIX_DIR, 'L5_chunk*_new_fixes.json')))
...
for fx in edits:
    ...
    if cell.value != old_val:
        print(f"  SKIP row {row_num} field {field}: expected {repr(old_val)!r}, got {repr(cell.value)!r}")
        skipped += 1
        continue
