"""Apply L5 third-pass audit fixes (JSON format) to words.xlsx."""
...
fix_files = sorted(glob.glob(os.path.join(FIX_DIR, 'L5_chunk*_p3_fixes.json')))
...
for fix in all_fixes:
    ...
    actual = cell.value or ''

    if actual != old_val:
        print(f"  SKIP row {row_num} field '{field}': expected {repr(old_val)!r}, got {repr(actual)!r}")
        skipped += 1
        continue
